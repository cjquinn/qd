...
namespace Qd {
    ...
    public:
        ...
        typename std::enable_if_t<(std::is_same_v<HandlerT<StateT, ActionT>, ReducersT> || ...), void>
        operator()(StateT &state, const ActionT &action) const {
            std::get<HandlerT<StateT, ActionT>>(reducers)(state, action);
        }

        template <class ActionT>
        typename std::enable_if_t<((std::is_same_v<HandlerT<StateT, ActionT>, ReducersT> || ...) == false), void>
        operator()(StateT &state, const ActionT &action) const {
            return;
        }
